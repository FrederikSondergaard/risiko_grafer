...
import matplotlib
matplotlib.use('Agg')
matplotlib.rcParams['font.family'] = ''  # eller 'Arial', hvis Helvetica ikke findes
...
print("Header file exists?", os.path.exists("assets/Header.png"))

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
HEADER_FILE = os.path.join(BASE_DIR, "assets", "Header.png")
FOOTER_FILE = os.path.join(BASE_DIR, "assets", "Footer.png")

HEADER_H = 50
FOOTER_H = 30

EXPL1 = ("Forklaring: Grafen viser en simulering af porteføljeudvikling baseret på 100.000 stokastiske ugentlige afkast over ti år."
        "Forventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger.")

EXPL2 = ("Forklaring: Grafen viser det forventede udfalsrum for porteføljens afkast p.a. baseret på et 95% konfidensinterval."
        "Forventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger.")
...
def create_figures(paths, start_beløb=1_000_000, mu=None, allocation=None):
    fig_allocation = create_allocation_figure(allocation or [{"asset": "I&T Aktier", "percent": 100}])
    ...
    return fig_allocation, fig1, fig2
...
def generate_pdf(fig_allocation, fig1, fig2):
    buffer = BytesIO()
    ...
    c.setFont("Helvetica", text_fontsize)
    c.setFillColorRGB(0.5, 0.5, 0.5)
    expl1_lines = [
        "Forklaring: Grafen viser en simulering af porteføljeudvikling baseret på 100.000 stokastiske ugentlige afkast over ti år.",
        "Forventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger."
    ]
    ...
    c.setFillColorRGB(0.5, 0.5, 0.5)
    expl2_lines = [
        "Forklaring: Grafen viser det forventede udfalsrum for porteføljens afkast p.a. baseret på et 95% konfidensinterval.",
        "Forventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger."
    ]
...
server = app.server  # <-- nødvendigt for Gunicorn og Azure Web Apps

asset_options = [
    {"label": "I&T Aktier", "value": "I&T Aktier"},
    {"label": "I&T Nalc", "value": "I&T Nalc"},
]

# Dynamisk render af allokerings-inputs
@app.callback(
    Output("allocation-container", "children"),
    Input("allocation-store", "data"),
)
def render_allocation_inputs(allocation):
    rows = []
    # Brug enumerate for at sikre at index altid er fortløbende
    for i, entry in enumerate(allocation):
        rows.append(
            html.Div([
                dcc.Dropdown(
                    id={"type": "asset-dropdown", "index": i},
                ...
                dcc.Input(
                    id={"type": "asset-percent", "index": i},
                    type="number",
                    value=entry["percent"],
                    min=0, max=100,
                    style={"width": "80px", "marginLeft": "10px", "display": "inline-block"}
                ),
                html.Button("✕", id={"type": "remove-asset", "index": i}, n_clicks=0, style={
                    "marginLeft": "10px", "color": "red", "fontWeight": "bold", "border": "none", "background": "none", "fontSize": "1.2em", "cursor": "pointer"
                }),
...
# Tilføj nyt aktiv
@app.callback(
    Output("allocation-store", "data"),
    Input("add-asset", "n_clicks"),
    State("allocation-store", "data"),
    prevent_initial_call=True,
)
def add_asset(n_clicks, allocation):
    allocation = allocation or []
    allocation.append({"asset": "I&T Aktier", "percent": 0})
    return allocation

# Opdater aktiv/allokering når dropdown eller procent ændres
@app.callback(
    Output("allocation-store", "data", allow_duplicate=True),
    Input({"type": "asset-dropdown", "index": dash.ALL}, "value"),
    Input({"type": "asset-percent", "index": dash.ALL}, "value"),
    State("allocation-store", "data"),
    prevent_initial_call=True,
)
def update_allocation(assets, percents, allocation):
    # Hvis allocation er None, lav en tom liste
    allocation = allocation or []
    n = max(len(assets), len(percents), len(allocation))
    new_allocation = []
    for i in range(n):
        # Brug eksisterende værdier hvis muligt
        asset = (
            assets[i] if i < len(assets) and assets[i] is not None
            else allocation[i]["asset"] if i < len(allocation)
            else "I&T Aktier"
        )
        try:
            percent = (
                int(percents[i]) if i < len(percents) and percents[i] not in (None, "")
                else allocation[i]["percent"] if i < len(allocation)
                else 0
            )
        except (ValueError, TypeError):
            percent = allocation[i]["percent"] if i < len(allocation) else 0
        new_allocation.append({"asset": asset, "percent": percent})
...
def remove_asset_callback(n_clicks_list, allocation):
    # Find hvilken knap der blev trykket på
    triggered = ctx.triggered_id
    if triggered is None or allocation is None:
        return allocation
    idx = triggered["index"]
    # Forhindre at alle aktiver kan fjernes
    if len(allocation) <= 1:
        return allocation
    # Returnér ny liste uden det valgte index
    new_allocation = [a for i, a in enumerate(allocation) if i != idx]
    return new_allocation

def create_allocation_figure(allocation):
    """
    allocation: list of dicts, e.g. [{"asset": "I&T Aktier", "percent": 60}, ...]
    """
    labels = [a["asset"] for a in allocation]
    sizes = [a["percent"] for a in allocation]
    fig, ax = plt.subplots(figsize=(7, 7))
    ax.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=plt.cm.Paired.colors)
    ax.set_title("AKTIVFORDELING", pad=40, fontsize=20, fontweight='bold')
    ax.axis('equal')
    # Fjern spines
...
def update_graph(allocation, start_str, mu_str, sigma_str):
    # Sikr allocation altid er en liste med mindst ét element
    if not allocation or len(allocation) == 0:
        allocation = [{"asset": "I&T Aktier", "percent": 100}]
    # Sikr at alle procentdele er tal og ikke None
    for a in allocation:
        if a["percent"] in (None, ""):
            a["percent"] = 0
    # Sikr at der er mindst én procentdel > 0
    if all(a["percent"] == 0 for a in allocation):
        return "", "", "", "", "", "Alle aktiver har 0% - fordel mindst én procentdel."
    try:
    ...
    try:
        ...
        fig_allocation, fig1, fig2 = create_figures(paths, start_beløb=start_beløb, mu=mu, allocation=allocation)
        src_allocation = fig_to_uri(fig_allocation)
        src1 = fig_to_uri(fig1)
        src2 = fig_to_uri(fig2)
    except Exception as e:
        return "", "", "", "", "", f"Fejl i grafgenerering: {e}"

    expl1 = "Forklaring: Grafen viser en simulering af porteføljeudvikling baseret på 100.000 stokastiske ugentlige afkast over ti år.\nForventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger."
    expl2 = "Forklaring: Grafen viser det forventede udfalsrum for porteføljens afkast p.a. baseret på et 95% konfidensinterval.\nForventninger til afkast og risiko er baseret på estimater fra Rådet for Afkastforventninger."

    return src_allocation, src1, src2, expl1, expl2, ""
...
def generate_pdf_callback(n_clicks, allocation, start_str, mu_str, sigma_str):
    try:
        start_renset = start_str.replace(".", "").replace(" ", "")
        start_beløb = int(start_renset)
        mu = parse_percent(mu_str)
        ...
        paths = simulate_paths(mu, sigma)
        fig_allocation, fig1, fig2 = create_figures(paths, start_beløb=start_beløb, mu=mu, allocation=allocation)
        pdf_data = generate_pdf(fig_allocation, fig1, fig2)
...
app.layout = html.Div([
    ...
    html.Br(),
    html.Label("INDTAST AKTIVFORDELING", style={"marginBottom": "50px", "marginLeft": "0px"}),
    html.Br(),
    html.Div(id="allocation-container", style={"marginBottom": "10px", "marginLeft": "15px"}),
    html.Button("+ Tilføj Aktivklasse", id="add-asset", n_clicks=0, style={"marginBottom": "20px", "marginLeft": "15px"}),
    dcc.Store(id="allocation-store", data=[{"asset": "I&T Aktier", "percent": 100}]), 
    html.Br(),
    ...
    html.A("Download PDF", id="download-link", href="", target="_blank", download="Risiko Rapport.pdf", style={"display": "none", "marginLeft": "20px"}),
    html.Div(id="error-msg", style={"color": "red", "textAlign": "center"}),
    html.Br(),
    html.Img(id="fig_allocation", style={"width": "500px", "display": "block", "margin": "0 auto"}),
    html.Div([
        html.Div([
